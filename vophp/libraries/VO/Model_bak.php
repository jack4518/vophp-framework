...
class VO_Model {
	...
	protected $_sql_methods = array('distinct', 'field', 'from', 'where', 'order', 'limit', 'page','having','group');
	...
	public function select(){
		$sql = $this->_parseMethod();
		
		$rows = $this->_read_db->fetchAll($sql);
		$this->_clearCallMetch();
		if($rows){
			return $rows;
		}else{
	 ...
	 */
	public function add($data, $where=null, $is_insert = true){
	 ...
	 * @param string $where  当为更新时的where条件
	 */
	public function addAll($datas, $where=null){
		$arrays = array();
		foreach($datas as $k => $data){
			$arrays[] = $this->add($data, $where, false);
		}
		$ret = $this->_write_db->insertAll($this->getTableName(), $arrays);
		if($ret === 0){
			return true;
		}else{
			return $ret;
...
<<<<<<< .mine
					        $where .= $join . $field . ' ' . strtoupper($cond[1]) . ' (';
=======
						    if(!empty($where)){
							    $where .= ' ' . $cond[2] . ' ';
						    }
